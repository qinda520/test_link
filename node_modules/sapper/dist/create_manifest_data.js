...
function create_serviceworker_manifest({ manifest_data, output, client_files, static_files }) {
    ...
    const code = `
		...
		export const files = [\n\t${files.map((x) => _commonjsHelpers.stringify('/' + x)).join(',\n\t')}\n];
		export { files as assets }; // legacy

		export const shell = [\n\t${client_files.map((x) => _commonjsHelpers.stringify('/' + x)).join(',\n\t')}\n];
...
function generate_server_manifest(manifest_data, path_to_routes, cwd, src, dest, dev) {
    const imports = [].concat(manifest_data.server_routes.map((route, i) => `import * as route_${i} from ${_commonjsHelpers.stringify(_commonjsHelpers.posixify(`${path_to_routes}/${route.file}`))};`), manifest_data.components.map((component, i) => `import * as component_${i} from ${_commonjsHelpers.stringify(get_file(path_to_routes, component))};`), `import * as root_comp from ${_commonjsHelpers.stringify(get_file(path_to_routes, manifest_data.root))};`, `import error from ${_commonjsHelpers.stringify(get_file(path_to_routes, manifest_data.error))};`);
    const component_lookup = {};
    manifest_data.components.forEach((component, i) => {
        component_lookup[component.name] = i;
    });
    const build_dir = _commonjsHelpers.posixify(path.normalize(path.relative(cwd, dest)));
    const src_dir = _commonjsHelpers.posixify(path.normalize(path.relative(cwd, src)));
    return `
		// This file is generated by Sapper — do not edit it!
		${imports.join('\n')}
...
var urlJoin = _commonjsHelpers.createCommonjsModule(function (module) {
...
var rollupPluginCssChunks = _commonjsHelpers.createCommonjsModule(function (module, exports) {
...
const url_join_1 = __importDefault(urlJoin);
...
const cssChunks = function (options = {}) {
    const filter = pluginutils_es.createFilter(/\.css$/i, []);
    ...
    return {
        ...
        load(id) {
            ...
            if (m !== null) {
                code = code.replace(m[0], '').trim();
                try {
                    map = fs.readFileSync(path_1.default.resolve(id, '..', m[1].trim()), 'utf8');
                }
                catch (err) {
                    console.warn(`Could not load css map file of ${id}.\n  ${err}`);
                }
            ...
            for (const chunk of Object.values(bundle).reverse()) {
                ...
                for (const f of css_modules) {
                    if (pluginOptions.sourcemap && emitFiles) {
                        ...
                        const decoded = sourcemapCodec.decode(css_data[f].map.mappings);
                        if (i === 0) {
                ...
                const css_file_url = url_join_1.default(pluginOptions.publicPath, css_file_name);
...
var css_chunks = /*@__PURE__*/_commonjsHelpers.unwrapExports(rollupPluginCssChunks);
...
class RollupResult {
    constructor(duration, compiler, sourcemap) {
        ...
        this.chunks = compiler.chunks.map(chunk => ({
            ...
            modules: Object.keys(chunk.modules).map(m => _commonjsHelpers.normalize_path(m))
        }));
        ...
        this.summary = compiler.chunks.map(chunk => {
            const size_color = chunk.code.length > 150000 ? _commonjsHelpers.$.bold().red : chunk.code.length > 50000 ? _commonjsHelpers.$.bold().yellow : _commonjsHelpers.$.bold().white;
            const size_label = _commonjsHelpers.left_pad(index$1.prettyBytes(chunk.code.length), 10);
    ...
    relative_dependencies(routes_dir) {
        ...
        Object.entries(this.dependencies).forEach(([key, value]) => {
            dependencies[_commonjsHelpers.normalize_path(path.relative(routes_dir, key)).replace(/\\/g, '/')] = value;
        });
        return dependencies;
    }
...
class RollupCompiler {
    ...
    get_config(mod) {
        return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
             ...
             */
            const sapper_internal = {
                name: 'sapper-internal',
                generateBundle(options, bundle) {
                    return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
                        function is_route(file_path) {
                            return file_path.includes(that.routes) && !file_path.endsWith('.css');
                        }
                        function js_deps(chunk, opts) {
                            return Array.from(index.dependenciesForTree(chunk, that.chunks, opts));
                        }
                        // Store the chunks for later use
                        Object.values(bundle)
                            .filter(c => c.type === 'chunk')
                            .forEach(c => that.chunks.push(c));
                        ...
                        // Routes dependencies
                        function add_dependencies(chunk) {
                            for (const module of Object.keys(chunk.modules)) {
                                if (is_route(module)) {
                                    let js_dependencies = js_deps(chunk, { walk: ctx => !ctx.dynamicImport && ctx.chunk.fileName !== entry_chunk.fileName }).map(c => c.fileName);
...
function create_compilers(bundler, cwd, src, routes, dest, dev) {
    return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
        ...
        env.set_dest(dest);
        try {
            env.set_module(JSON.parse(fs.readFileSync(path.resolve(cwd, 'package.json'), 'utf-8')).type === 'module');
        }
        catch (err) {
            env.set_module(false);
        }
        if (bundler === 'rollup') {
...
function create_manifest_data(cwd, extensions = '.svelte .html') {
    ...
    function find_layout(file_name, component_name, dir = '') {
        ...
        const file = _commonjsHelpers.posixify(path.join(dir, `${file_name}${ext}`));
        return ext
            ? {
                name: component_name,
                file
