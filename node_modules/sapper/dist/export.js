...
function exportQueue({ concurrent, handleFetch, handleResponse, fetchOpts, callbacks }) {
    ...
    function processQueue() {
        return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
            ...
            saving = yield filterNotPending(saving);
            // move resolved from fetching queue and into saving queue until saving queue is full
            let fetchedIndex = yield findNotPendingIndex(fetching);
            while (saving.length < concurrent && fetchedIndex !== -1) {
                ...
                fetchedIndex = yield findNotPendingIndex(fetching);
            }
...
function _export({ cwd, static: static_files = 'static', build_dir = '__sapper__/build', export_dir = '__sapper__/export', basepath = '', host_header = undefined, timeout = 5000, concurrent = 8, oninfo = fs_utils.noop, onfile = fs_utils.noop, entry = '/' } = {}) {
    return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
        ...
        fs_utils.rimraf(export_dir);
        fs_utils.copy(static_files, export_dir);
        fs_utils.copy(path.join(build_dir, 'client'), path.join(export_dir, 'client'));
        const has_serviceworker = fs.existsSync(path.join(build_dir, 'service-worker.js'));
        if (has_serviceworker) {
            fs_utils.copy(path.join(build_dir, 'service-worker.js'), path.join(export_dir, 'service-worker.js'));
            fs_utils.copy(path.join(build_dir, 'service-worker.js.map'), path.join(export_dir, 'service-worker.js.map'));
        }
        const defaultPort = process.env.PORT ? parseInt(process.env.PORT) : 3000;
        const port = yield Deferred.find(defaultPort);
        const protocol = 'http:';
        ...
        function save(url, status, type, body) {
            ...
            if (is_html) {
                ...
                if (typeof body === 'string') {
                    body = minify_html.minify_html(body);
                }
                else {
            ...
            if (fs.existsSync(export_file))
                return;
            fs_utils.mkdirp(path.dirname(export_file));
        ...
        function handleFetch(url, opts) {
            return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
                const href = url.href;
                const timeout_deferred = new Deferred.Deferred();
        ...
        function handleResponse(fetched, fetchOpts) {
            return _commonjsHelpers.__awaiter(this, void 0, void 0, function* () {
                const { response, url } = yield fetched;
                let pathname = url.pathname;
                if (pathname !== '/service-worker-index.html') {
                    pathname = pathname.replace(fetchOpts.root.pathname, '') || '/';
                }
                ...
                if (range === 2 && type === 'text/html') {
                    ...
                    const link = httpLinkHeader.parse(response.headers.get('Link') || '');
                    link.refs.forEach((ref) => {
                        if (ref.rel === 'preload' || ref.rel === 'modulepreload') {
                            body = body.replace('</head>', `<link rel=${JSON.stringify(ref.rel)} as=${JSON.stringify(ref.as)} href=${JSON.stringify(ref.uri)} ${ref.as === 'script' ? 'crossorigin="use-credentials"' : ''}></head>`);
                        }
                    });
                    if (pathname !== '/service-worker-index.html') {
...
exports.get_srcset_urls = get_srcset_urls;
